...
public class AllUsermsg extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("all")!=null){
			...
			all=sqlSession.getMapper(UserMapper.class).getCount();
			sqlSession.close();
		}
		page=all%3 == 0 ? (all/3) : ((all/3)+1);
		List<usermsg> list=new ArrayList<usermsg>();
		SqlSession sqlSession=MybatisReader.getSession();
		list=sqlSession.getMapper(UserMapper.class).getPage(count*3);
		sqlSession.close();
		if(list.size()!=0){
			out.println("<table>");
			out.println("<th>用户名</th><th>性别</th><th>手机</th><th>邮箱</th><th>地址</th>");
		...
		if(count>=(page-1)){
		 ...
		 out.println("<tr id='tr_bottom'><td colspan='5'><a id="+pre+">上一页</a>&nbsp;&nbsp;");
		 ...
		 out.println("<tr>");
		 out.println("<td colspan='5'><button type='submit' class='btn btn-default' id='add'>新增员工</button></td>");
		 out.println("<tr>");
		 out.println("</table>");
		}
		out.flush();
